-- =====================================================================
--  MANONMANI - SHOP PC DATABASE SETUP
--  shop_id: manonmani              Everything is filled in. Nothing to edit.
--
...
--
--                   Manonmani has two machines in the licence list. Run
--                   this, and install the sync agent, on the one that
--                   HOLDS the pawnbroking database.
--
--  ORDER            1. manonmani_cloud_provision.sql on Railway, R1-R6
--                   2. Install the sync agent on the shop PC (below)
--                   3. F5 on the shop PC             (this file)
--                   4. manonmani_cloud_verify.sql on Railway
...
--  notice_one_per_bill_setting.sql, and initial_full_backfill.sql (built
--  on the balamurugan_full_backfill pattern instead, which is the one
--  that stops repledges collapsing).
--
--  Not included on purpose: ledger_day_enforcement.sql. It refuses
--  back-dated entries, which is a separate decision for this shop.
-- =====================================================================
...
--
--    1. Run ShopInstallerBundle\SyncInstaller\dist\PawnBrokingSyncSetup.exe
--       as administrator and fill in:
--           Shop ID        manonmani
--           Cloud API key  the mbk_ key from R5 of the cloud file
--           Cloud URL      leave the default
--           DB user        postgres
--           DB password    the one pgAdmin connects with on this PC
--    2. C:\ProgramData\PawnBroking\sync.properties must then say
--           shop.id=manonmani
...
--
--  Run it in pgAdmin, and when NOBODY IS BILLING. Sending the whole
--  history takes a few minutes as one step, and until it finishes, saving
--  a bill waits and the repledge screens freeze. It is not stuck - do not
--  cancel. Tools with a statement time limit will cancel it for you: the
--  Magizhchi DB Communicator stops every statement at 30 seconds, and
--  this one needs longer. A cancelled run changes nothing; just run it
--  again here.
-- ---------------------------------------------------------------------
...
BEGIN
    ...
    IF v_capture IS DISTINCT FROM 'pk-aware' THEN
        PERFORM set_config('mn.history',
            'WAITING - the sync agent on this PC is out of date and would send rows with the wrong keys. Run update-agent.bat from the agent folder as administrator, wait 30 seconds, press F5 again.', false);
        RETURN;
    END IF;
    ...
    IF v_mark LIKE 'manonmani history queued%' THEN
        PERFORM set_config('mn.history',
    ...
    IF NOT EXISTS (SELECT 1 FROM pg_index
        ...
        IF v_dupes > 0 OR v_nulls > 0 THEN
            PERFORM set_config('mn.history', format(
    ...
    PERFORM set_config('app.shop_id', 'manonmani', false);
    ...
    EXECUTE format('COMMENT ON TABLE sync_outbox IS %L',
                   format('manonmani history queued %s, %s rows',
...
BEGIN
    ...
    -- on the phone as marunganathan@gmail.com. Until then every upload
    -- gets 503.
    v_txt := NULL;
    ...
    END IF;
    PERFORM set_config('mn.photo_dirs', COALESCE(v_txt, '(none set)'), false);
    ...
    END IF;
    PERFORM set_config('mn.backup_dirs', COALESCE(v_txt, '(none set)'), false);

    IF to_regclass('public.sync_outbox') IS NOT NULL THEN
        EXECUTE 'SELECT count(*) FROM sync_outbox WHERE sent_at IS NULL' INTO n;
        PERFORM set_config('mn.pending', n::text, false);
    ELSE
        PERFORM set_config('mn.pending', '- (no agent yet)', false);
    END IF;
    IF to_regclass('public.sync_image_uploads') IS NOT NULL THEN
        EXECUTE 'SELECT count(*) FROM sync_image_uploads' INTO n;
        PERFORM set_config('mn.images', n::text, false);
    ELSE
        PERFORM set_config('mn.images', '- (no agent yet)', false);
    END IF;
    IF to_regclass('public.sync_backup_uploads') IS NOT NULL THEN
        EXECUTE 'SELECT count(*) FROM sync_backup_uploads' INTO n;
        PERFORM set_config('mn.backups', n::text, false);
    ELSE
        PERFORM set_config('mn.backups', '- (no agent yet)', false);
    END IF;

    -- What manonmani_cloud_verify.sql should match once the queue drains.
    EXECUTE 'SELECT count(*) FROM company_billing' INTO n;
    PERFORM set_config('mn.rows_bills', n::text, false);
    IF to_regclass('public.repledge_billing') IS NOT NULL THEN
        EXECUTE 'SELECT count(*) FROM repledge_billing' INTO n;
        PERFORM set_config('mn.rows_repledge', n::text, false);
    ELSE
        PERFORM set_config('mn.rows_repledge', '(no table)', false);
    END IF;
    IF to_regclass('public.customer_details') IS NOT NULL THEN
        EXECUTE 'SELECT count(*) FROM customer_details' INTO n;
        PERFORM set_config('mn.rows_customers', n::text, false);
    ELSE
        PERFORM set_config('mn.rows_customers', '(no table)', false);
    END IF;
END $$;
...
SELECT step, item, status FROM (
    ...
    (8,  'History to the cloud',   current_setting('mn.history', true)),
    (9,  'Waiting to send',        current_setting('mn.pending', true)),
    (10, 'Photos uploaded so far', current_setting('mn.images', true)),
    (11, 'Backups uploaded so far', current_setting('mn.backups', true)),
    (12, 'Photo folder(s) - must exist on THIS PC',  current_setting('mn.photo_dirs', true)),
    (13, 'Backup folder(s) - must exist on THIS PC', current_setting('mn.backup_dirs', true)),
    (14, 'Desktop rows: company_billing',  current_setting('mn.rows_bills', true)),
    (15, 'Desktop rows: repledge_billing', current_setting('mn.rows_repledge', true)),
    (16, 'Desktop rows: customer_details', current_setting('mn.rows_customers', true))
