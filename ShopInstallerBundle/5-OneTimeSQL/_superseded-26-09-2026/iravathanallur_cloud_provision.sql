-- =====================================================================
--  IRAVATHANALLUR — CLOUD PROVISIONING
--  Run on Railway -> your cloud service -> Data -> Query.
--  Run each statement ONE AT A TIME (the console splits on ';').
--
--  Iravathanallur already runs the DESKTOP app. This adds the CLOUD
--  side, which is what the mobile app and a Meet "Pawn Shop" room read
--  from. Until this is done the shop works normally on its own PC and is
--  simply invisible everywhere else.
--
--  Nothing to fill in. Three addresses may sign in:
--     rajeshwariiravathanallur@gmail.com  the owner, this branch only
...
-- 5. (Railway UI, not SQL) Add  iravathanallur  to the TENANTS variable,
--    comma-separated, and let it redeploy (~3 min). Confirm in Deploy
--    Logs:  "Provisioning tenant schema 'iravathanallur'".
...
--       changes. Run initial_full_backfill.sql (shop_id iravathanallur)
--       to push the history, and expect it to take a while.
...
--  "Pawn shop this room shows" = iravathanallur, and the owner's address
...
--  publish, and reaches only this room). The PRAMA recorder is set up on that room
--  with "This room has its own recorder" ticked — see the runbook.
-- =====================================================================
