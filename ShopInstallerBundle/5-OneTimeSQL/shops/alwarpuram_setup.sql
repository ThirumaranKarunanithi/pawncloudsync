-- =====================================================================
--  ALWARPURAM PAWN BROKING  -  COMPLETE SETUP, CLOUD + SHOP PC
--  shop_id: alwarpuram
--
...
--
--  REPLACES, for this shop: alwarpuram_cloud_provision.sql,
--  alwarpuram_cloud_verify.sql, the backfills and replays,
...
/* =====================================================================
   ...
   ALREADY PROVISIONED. R1 to R5 were done when alwarpuram went live, so
...
VALUES ('alwarpuram', 'alwarpuram', 'Alwarpuram Pawn Broking')
ON CONFLICT DO NOTHING;

-- R2/R3  This shop went live before these files existed, so its
--        owner address is whatever is already in the cloud. Read it
--        rather than setting it - overwriting it would lock the
--        owner out of the phone.
SELECT primary_email AS owner_on_record
  FROM public.tenants WHERE shop_id = 'alwarpuram';

-- R4  The two admin addresses that hold every Rajeshwari branch.
INSERT INTO public.user_shop_access (email, shop_id, role)
VALUES ('tirukaruna@gmail.com', 'alwarpuram', 'OWNER')
ON CONFLICT (email, shop_id) DO NOTHING;

INSERT INTO public.user_shop_access (email, shop_id, role)
VALUES ('neelamanikandank@gmail.com', 'alwarpuram', 'OWNER')
ON CONFLICT (email, shop_id) DO NOTHING;
...
--        WHERE shop_id = 'alwarpuram' AND revoked_at IS NULL;
...
--         'alwarpuram', 'Alwarpuram Pawn Broking - sync agent')
-- RETURNING api_key;

--     Then, in the Railway UI (not SQL): add  alwarpuram  to the TENANTS
--     variable, comma-separated, and let it redeploy (~3 min). Deploy
--     Logs should show:  Provisioning tenant schema 'alwarpuram'

-- R6  Check. Expect 1 tenant, an owner email, the sign-ins, 1 api key.
SELECT 'tenant'  AS what, shop_id                     AS value FROM public.tenants          WHERE shop_id='alwarpuram'
UNION ALL
SELECT 'email',  COALESCE(primary_email,'(none)')            FROM public.tenants          WHERE shop_id='alwarpuram'
UNION ALL
SELECT 'access', email || ' (' || role || ')'                FROM public.user_shop_access WHERE shop_id='alwarpuram' AND revoked_at IS NULL
UNION ALL
SELECT 'api_key',label                                       FROM public.shop_credentials WHERE shop_id='alwarpuram' AND revoked_at IS NULL;
...
-- DELETE FROM alwarpuram.projections WHERE table_name = 'repledge_billing';
...
BEGIN
    ...
    IF array_length(v_done, 1) IS NOT NULL
       ...
       AND EXISTS (SELECT 1 FROM pg_trigger WHERE tgname = 'trg_sync_company_billing') THEN
        PERFORM set_config('app.shop_id', 'alwarpuram', false);
...
BEGIN
    ...
    EXECUTE format('COMMENT ON TABLE sync_outbox IS %L',
                   format('alwarpuram history queued %s, %s rows',
