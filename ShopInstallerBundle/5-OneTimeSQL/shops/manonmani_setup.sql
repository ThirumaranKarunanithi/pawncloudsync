-- =====================================================================
--  MANONMANI PAWN BROKING  -  COMPLETE SETUP, CLOUD + SHOP PC
--  shop_id: manonmani
--
...
--
--  REPLACES, for this shop: manonmani_cloud_provision.sql,
--  manonmani_cloud_verify.sql, the backfills and replays,
...
/* =====================================================================
   ...
   ALREADY PROVISIONED. R1 to R5 were done when manonmani went live, so
...
INSERT INTO public.user_shop_access (email, shop_id, role)
VALUES ('tirukaruna@gmail.com', 'manonmani', 'OWNER')
ON CONFLICT (email, shop_id) DO NOTHING;
...
--         'manonmani', 'Manonmani Pawn Broking - sync agent')
-- RETURNING api_key;

--     Then, in the Railway UI (not SQL): add  manonmani  to the TENANTS
--     variable, comma-separated, and let it redeploy (~3 min). Deploy
--     Logs should show:  Provisioning tenant schema 'manonmani'
...
-- DELETE FROM manonmani.projections WHERE table_name = 'repledge_billing';
