-- =====================================================================
--  AIYANARPURAM PAWN BROKING  -  COMPLETE SETUP, CLOUD + SHOP PC
--  shop_id: aiyanarpuram
--
...
--
--  REPLACES, for this shop: aiyanarpuram_cloud_provision.sql,
--  aiyanarpuram_cloud_verify.sql, the backfills and replays,
...
/* =====================================================================
   ...
   ALREADY PROVISIONED. R1 to R5 were done when aiyanarpuram went live, so
...
--        WHERE shop_id = 'aiyanarpuram' AND revoked_at IS NULL;
...
--         'aiyanarpuram', 'Aiyanarpuram Pawn Broking - sync agent')
-- RETURNING api_key;

--     Then, in the Railway UI (not SQL): add  aiyanarpuram  to the TENANTS
--     variable, comma-separated, and let it redeploy (~3 min). Deploy
--     Logs should show:  Provisioning tenant schema 'aiyanarpuram'
...
-- DELETE FROM aiyanarpuram.projections WHERE table_name = 'repledge_billing';
...
BEGIN
    ...
    IF array_length(v_done, 1) IS NOT NULL
       ...
       AND EXISTS (SELECT 1 FROM pg_trigger WHERE tgname = 'trg_sync_company_billing') THEN
        PERFORM set_config('app.shop_id', 'aiyanarpuram', false);
...
BEGIN
    ...
    EXECUTE format('COMMENT ON TABLE sync_outbox IS %L',
                   format('aiyanarpuram history queued %s, %s rows',
