-- =====================================================================
--  DHINESHSUGANYA PAWN BROKING  -  COMPLETE SETUP, CLOUD + SHOP PC
...
--
--  REPLACES, for this shop: dhineshsuganya_cloud_provision.sql,
--  dhineshsuganya_cloud_verify.sql, the backfills and replays,
...
/* =====================================================================
   ...
   NOT YET PROVISIONED. Run R1 to R6 in order, one statement at a
   time, BEFORE the shop PC half - R5 hands you the key the agent
   needs in sync.properties.
...
INSERT INTO public.user_shop_access (email, shop_id, role)
VALUES ('tirukaruna@gmail.com', 'dhineshsuganya', 'OWNER')
ON CONFLICT (email, shop_id) DO NOTHING;
...
--         'dhineshsuganya', 'Dhineshsuganya Pawn Broking - sync agent')
-- RETURNING api_key;

--     Then, in the Railway UI (not SQL): add  dhineshsuganya  to the TENANTS
--     variable, comma-separated, and let it redeploy (~3 min). Deploy
--     Logs should show:  Provisioning tenant schema 'dhineshsuganya'
...
-- DELETE FROM dhineshsuganya.projections WHERE table_name = 'repledge_billing';
...
BEGIN
    ...
    IF array_length(v_done, 1) IS NOT NULL
       ...
       AND EXISTS (SELECT 1 FROM pg_trigger WHERE tgname = 'trg_sync_company_billing') THEN
        PERFORM set_config('app.shop_id', 'dhineshsuganya', false);
...
BEGIN
    ...
    EXECUTE format('COMMENT ON TABLE sync_outbox IS %L',
                   format('dhineshsuganya history queued %s, %s rows',
...
SET app.shop_id = 'dhineshsuganya';
