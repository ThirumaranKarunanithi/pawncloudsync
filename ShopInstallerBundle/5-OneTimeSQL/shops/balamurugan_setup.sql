-- =====================================================================
--  BALAMURUGAN PAWN BROKING  -  COMPLETE SETUP, CLOUD + SHOP PC
--  shop_id: balamurugan
--
...
--
--  REPLACES, for this shop: balamurugan_cloud_provision.sql,
--  balamurugan_cloud_verify.sql, the backfills and replays,
...
/* =====================================================================
   ...
   ALREADY PROVISIONED. R1 to R5 were done when balamurugan went live, so
...
-- R4  No second address on purpose. This is the one shop that does
--     NOT get the Rajeshwari admin logins - it is its own business.
--     Do not add them without being asked.
...
--        WHERE shop_id = 'balamurugan' AND revoked_at IS NULL;
...
--         'balamurugan', 'Balamurugan Pawn Broking - sync agent')
-- RETURNING api_key;

--     Then, in the Railway UI (not SQL): add  balamurugan  to the TENANTS
--     variable, comma-separated, and let it redeploy (~3 min). Deploy
--     Logs should show:  Provisioning tenant schema 'balamurugan'
...
UNION ALL
SELECT 'access', email || ' (' || role || ')'                FROM public.user_shop_access WHERE shop_id='balamurugan' AND revoked_at IS NULL
UNION ALL
...
-- DELETE FROM balamurugan.projections WHERE table_name = 'repledge_billing';
...
BEGIN
    ...
    IF array_length(v_done, 1) IS NOT NULL
       ...
       AND EXISTS (SELECT 1 FROM pg_trigger WHERE tgname = 'trg_sync_company_billing') THEN
        PERFORM set_config('app.shop_id', 'balamurugan', false);
...
BEGIN
    ...
    EXECUTE format('COMMENT ON TABLE sync_outbox IS %L',
                   format('balamurugan history queued %s, %s rows',
